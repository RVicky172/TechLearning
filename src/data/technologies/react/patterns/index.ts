...
export const reactPatterns: TopicNode = {
  ...
  iconName: "Shapes",
  theory:
    "Beyond the core building blocks, React has a rich set of patterns for solving recurring problems — code reuse, cross-cutting concerns, DOM escape hatches, resilience, and performance. These patterns range from legacy HOCs and render props to modern Portals, Error Boundaries, lazy loading, ref forwarding, and Compound Components.",
  theoryDetail: {
    keyConcepts: [
      "HOCs and render props are older patterns — still common in the ecosystem but usually replaceable with custom hooks",
      "Compound Components create highly flexible, multi-part UI widgets (like Tabs or Menus) that share state implicitly",
    ...
    whyItMatters:
      "Real production applications need more than just components and state. They need resilience (Error Boundaries), performance at scale (lazy loading), interoperability with the DOM (Portals, forwardRef), and ways to share cross-cutting logic across many components (HOCs, custom hooks, Compound Components). Mastering these patterns distinguishes senior React engineers.",
    commonPitfalls: [
  ...
  children: [hoc, renderProps, portals, lazyLoading, errorBoundaries, testing, forwardRef, compoundComponents, componentDesign],
};
